...
train_images_list = os.listdir("./train")

def read_image(filename):
    image = Image.open("./train/" + filename).convert("RGB")
    new_img = image.resize((32, 32), Image.BILINEAR)
...
for filename in train_images_list:
    x_train.append(read_image(filename))
    y_train.append(1)
...
#     for img in samples:
    ...
    for idx, img in enumerate(samples):
        ...
        plt.savefig(str(epochs)+str(steps)+str(i)+".png")
        plt.show()
        ...
        for e in range(epochs):
            for batch_i in range(images.shape[0]//batch_size-1):
                ...
                if steps % 100 == 0:
                    ...
                    np.save('sample_res.npy', samples)
                    print("save complete")

                    plot_images(samples,epochs,steps)
...
samples = np.load("sample_res.npy")
...
def save_images(samples,epochs,steps):
    samples = (samples + 1) / 2
#     for img in samples:
    i = 0
    for idx, img in enumerate(samples):
...
#         img.save('output_%d.png' % idx)

save_images(samples,0,1)
...
fig.savefig('ax2_figure_expanded.png')
